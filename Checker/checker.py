...
zjm = pd.read_excel(resultPath+rf"\out_{Instance_name}.xlsx", sheet_name="zjm")
zjm = np.array(zjm)[:, 1:]
pim = pd.read_excel(resultPath+rf"\out_{Instance_name}.xlsx", sheet_name="pim")
pim = np.array(pim)[:, 1:]
# manual:Only use Instances with the prefix QX
# zjm = pd.read_excel(resultPath + rf"\manual_{Instance_name}.xlsx", sheet_name="zjm")
# zjm = np.array(zjm)[:, 1:]
# pim = pd.read_excel(resultPath + rf"\manual_{Instance_name}.xlsx", sheet_name="pim")
# pim = np.array(pim)[:, 1:]

temp_path = filePath + fr'\{Instance_name}'
df1 = pd.read_parquet(temp_path + r'\Classroom_capacity.parquet')
...
C_i = C_i[0:I]
# print('CI', C_i)
df2 = pd.read_parquet(temp_path + r'\Student_course_matrix.parquet')
...
df3 = pd.read_parquet(temp_path + r'\Classroom_region_matrix.parquet')
T_ir = df3.iloc[0:I, 0:R].values.tolist()
df5 = pd.read_parquet(temp_path + r'\Teacher_course_matrix.parquet')
Q_km = df5.iloc[0:, 0:M].values.tolist()

df = pd.read_parquet(temp_path + r'\same_time_Teacher_course_matrix.parquet')
...
for i in range(0, len(TK)):
    for k in range(0, len(TK[i])):
        TK[i][k] = int(TK[i][k])
df4 = pd.read_parquet(temp_path + r'\Classroom_campus_matrix.parquet')
df6 = pd.read_parquet(temp_path + r'\course_campus_matrix.parquet')
df7 = pd.read_parquet(temp_path + r'\Limit_invigilators.parquet')
# check
...
def check_feasible(I, J, M, S, H_sm, K, Q_km, TK, zjm, pim, T_ir):
    ...
    temp_ji_num = zjm @ pim.T
    for j in range(J):
        if fsum(temp_ji_num[j]) > df7.iloc[j][0]:
            return False, f"Constraint violated: time slot j={j} exceed limited number of invigilators {df7.iloc[j][0]}"
